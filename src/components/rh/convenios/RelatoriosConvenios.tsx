...
export function RelatoriosConvenios() {
  const [dadosRelatorio, setDadosRelatorio] = useState<RelatorioConvenios[]>([]);
  ...
  const fetchDadosRelatorio = async () => {
    try {
      ...
      const agrupado = data?.reduce((acc: any, item: any) => {
        const tipo = item.convenio?.tipo || 'outros';
        if (!acc[tipo]) {
          acc[tipo] = { tipo, total_colaboradores: 0, custo_total: 0 };
        }
        acc[tipo].total_colaboradores += 1;
        acc[tipo].custo_total += Number(item.valor_mensal) || 0;
        return acc;
      }, {});
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Relatórios de Convênios</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Colaboradores por Tipo</CardTitle>
          </CardHeader>
          <CardContent>
